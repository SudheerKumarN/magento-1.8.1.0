...
class Ct_SetBackground_Block_Extensions extends Mage_Adminhtml_Block_System_Config_Form_Fieldset {
    ...
    public function render(Varien_Data_Form_Element_Abstract $element) {
        ...
        foreach ($modules as $moduleName) {
            ...
            if (strstr($moduleName, 'Ct_') === false) {
                continue;
            }
    ...
    protected function _getFieldHtml($fieldset, $moduleCode) {
        ...
        $status = '<a  target="_blank"><img src="' . $this->getSkinUrl('images/ct-logo.png') . '" title="' . $this->__("Installed") . '"/></a>';
        ...
        if ($allExtensions && isset($allExtensions[$moduleCode])) {
            ...
            if ($this->_convertVersion($currentVer) < $this->_convertVersion($lastVer)) {
                $status = '<a href="' . $url . '" target="_blank"><img src="' . $this->getSkinUrl('images/ct-logo.png') . '" alt="' . $this->__("Update available") . '" title="' . $this->__("Update available") . '"/></a>';
            }
        }
